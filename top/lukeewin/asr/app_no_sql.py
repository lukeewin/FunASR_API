...
# 全局内存缓存：用于替代数据库持久化
# 结构设计：{ task_id: { "all_content": str, "sentences": list } }
asr_results_cache = {}
...
async def result(task_id: str = Form(..., description="任务ID")):
    try:
        # 直接从内存缓存中获取并删除（pop 保证结果只被取一次，避免内存无限堆积）
        task_result = asr_results_cache.pop(task_id, None)
        ...
        if task_result:
            return response_format(
                ...
                message="获取结果成功", 
                data={
                    "all_content": task_result["all_content"], 
                    "sentences": task_result["sentences"]
                }
            )
        else:
            return response_format(code=303, status="error", message="获取结果为空或任务仍在处理中")
    except Exception as e:
        ...
        try:
            ...
            if trans_result:
                ...
                for sentence in sentence_info:
                    ...
                    # 字段名映射保持与你原本从数据库查出的字段一致: sentence_index, text, start, end, spk
                ...
                # 将完整结果整合成一个字典直接丢入内存缓存
                asr_results_cache[task_id] = {
                    "all_content": all_content,
                    "sentences": sentences_list
                }
                print(f"{task_id} - 数据已保存到内存缓存")
            else:
                print(f"{task_id} - 没有转写结果")
        except Exception as e:
            print(f"{task_id} - 异步后台任务异常: {e}")
